    ...
    void OnSceneGUI()
    {
        if (EditProbesMode) {
            ...
            for (int i = 0; i < probes_Pos.Length; i++)
            {
                ...
                if (myTarget.tempList.Contains(i))
                {
                    ...
                    Handles.SphereHandleCap(0, probes_Pos[i], Quaternion.identity, .6f, EventType.Repaint);
                }
                //Is not in linked group - Red
                else
                {
                    Handles.color = Color.red;
                    Handles.SphereHandleCap(0, probes_Pos[i], Quaternion.identity, .55f, EventType.Repaint);
                }
                ...
                if (Vector3.Distance(probes_Pos[i], positionHandle) < selectRange)
                {
                    ...
                    Handles.SphereHandleCap(0, probes_Pos[i], Quaternion.identity, .4f, EventType.Repaint);
                }
            }
        }
        else
        {
            ...
            if(myTarget.tempList != null)
            {
                ...
                {
                    Handles.SphereHandleCap(0, probes_Pos[myTarget.tempList[i]], Quaternion.identity, .2f, EventType.Repaint);
                }
            }
        }
    }
